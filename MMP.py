    ...
    def create_bus_frames(self):
        if self.root != None:
            for i in range(3):
                ...
                number_frame.grid(row=self.coords[0] + self.rowspan - 1 - 2*i, column=self.coords[1], rowspan=2, sticky="NEWS")
                ...
                terminus_frame.grid(row=self.coords[0] + self.rowspan - 1 - 2*i, column=self.coords[1]+1, columnspan=self.colspan - 1, sticky="NEWS")
                ...
                time_frame.grid(row=self.coords[0] + self.rowspan - 2*i, column=self.coords[1]+1, columnspan=self.colspan - 1, sticky="NEWS")
        ...
        if self.root != None:
            for i in range(3):
                ...
                if len(self.schedule) > i:
                    self.number_frames[row].configure(text=self.schedule[i][0], fg=BUS_COLOUR)
                    if self.schedule[i][3] == "trol":
                        self.number_frames[row].configure(fg=TROLLEY_COLOUR)
                    self.terminus_frames[row].configure(text=self.schedule[i][1])
...
class WeatherController:
    ...
    def create_widgets(self):
        if self.root != None:
            ...
            temp_frame.grid(row=self.coords[0]+1, column=self.coords[1]+1, columnspan=3, sticky="NEWS")
            ...
            feels_frame.grid(row=self.coords[0]+2, column=self.coords[1]+2, columnspan=2, sticky="NEWS")
            self.widgets.append(feels_frame)

            icon_frame = tk.Label(
                self.root, text="Icon", bg=DEFAULT_BACKGROUND_COLOUR, fg=DEFAULT_ACCENT_COLOUR, anchor="e")
            icon_frame.grid(row=self.coords[0]+1, column=self.coords[1], sticky="NEWS")
            ...
            wind_direction_frame.grid(row=self.coords[0], column=self.coords[1]+2, columnspan=2, sticky="NEWS")
            self.widgets.append(wind_direction_frame)

            wind_speed_frame = tk.Label(
                self.root, text="Wind", bg=DEFAULT_BACKGROUND_COLOUR, fg=DEFAULT_ACCENT_COLOUR,
                font=(FONT, WIND_SIZE), anchor="e")
            wind_speed_frame.grid(row=self.coords[0], column=self.coords[1]+1, columnspan=2, sticky="NEWS")
        ...
        if len(self.widgets) > 0:
            for i in range(min(len(self.widgets), len(self.data))):
                if i == 2:
                    ...
                    self.widgets[i].configure(text=self.convert_angle_to_dir(self.data[i]))
                elif i == 4:
                    self.widgets[i].configure(text=f"{round(self.data[i], 1)}m/s")
                else:
...
class TimeController:
    ...
    def create_clock_widget(self):
        if self.root != None:
            self.widget = tk.Label(self.root, font=(FONT, CLOCK_FONT_SIZE),
                            bg=DEFAULT_BACKGROUND_COLOUR, fg=DEFAULT_FONT_COLOUR, anchor="nw")
            self.widget.grid(row=self.coords[0], column=self.coords[1], rowspan=self.rowspan, columnspan=self.colspan, sticky="NEWS")
    ...
    def create_date_frame(self):
        if self.root != None:
            self.widget = tk.Label(self.root, font=(FONT, DATE_FONT_SIZE),
                                bg=DEFAULT_BACKGROUND_COLOUR, fg=DEFAULT_FONT_COLOUR, anchor="nw")
            self.widget.grid(row = self.coords[0]+1, column=self.coords[1], rowspan=self.rowspan, columnspan=self.colspan, sticky="NEWS")
...
class Program:
    def __init__(self):
        ...
        self.timeController = TimeController(coords=(0, 0), root=self.root, rowspan=2, colspan=10)
        self.dateController = DateController(coords=(1, 0), root=self.root, rowspan=1, colspan=10)
        self.busController = BusController(coords=(5,0), root=self.root, rowspan=6, colspan=8)
        self.weatherController = WeatherController(coords=(0,12), root=self.root)
